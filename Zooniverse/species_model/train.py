...
def train_model(train_path, test_path, empty_images_path=None, save_dir=".", debug = False):
    ...
    dataloader = torch.utils.data.DataLoader(ds, batch_size = model.config["batch_size"], shuffle=True, sampler = sampler, collate_fn=utilities.collate_fn, num_workers=model.config["workers"])
