...
To estimate the date of first egg lay, we assume that eggs are generally laid every other day and always use the oldest egg or chick when generating our estimates.
The following rules are applied to the nest check data to estimate first lay date: 
...
Determining initiation date has changed over time. We have enforced standardization on what is early, mid, or late month and how to translate between the observed nest state and egg lay estimation. In a few cases, this resulted in changing the reported date score. This is documented in the "corrected" column.
...
#### Rules to choose date from flight observations  

* paired or nest starts -> assume incubation occurred shortly thereafter  
  * Therefore use date to set early, mid, late. If date on edge time periods, then push to next time period.  
* incubating, but only a few seen -> assume these are your first egg lays and was recent.  
  * Choose flight date to set time period  
* incubating but many are incubating -> assume observation is occurring after first egg lay  
  * Remove two weeks from date and set time period  
* incubating but no info -> flight date sets time period  
* young/small chicks -> remove 1 month from observation date and set time period   
* young storks moving on/off nests -> remove 90 days  
* unknown state -> if only know nests seen and not other info, then choose midpoint between flights  
  * If no earlier flight conducted, then date sets time period unless on border between time periods, then push to earlier time period.  
* If the person who saw the nest disagrees with the assessment, then change time period to match their conclusion.   
  * Document in notes why they thought it was mid/early/late instead of what our rule set would suggest.
